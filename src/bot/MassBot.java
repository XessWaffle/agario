...
public class MassBot extends Player {
	...
	public MassBot(World in, String name, Color cellColor) {
	...
	public void step() {
		...
		if(isDead()) {
			...
			try {
				...
				controller.setError((in.getLeaderboard().get(in.getLeaderboard().size() - 1).getTotalMass() - this.getTotalMass())/(in.getLeaderboard().get(in.getLeaderboard().size() - 1).getTotalMass()) + (2 * in.getPosition(this)/(double)in.getPlayers().size()));
