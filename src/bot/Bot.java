...
public class Bot extends Player {
	...
	public static final int EXTENDED_VIEW = 0;
	private NeuralNetwork controller;
	protected Zone toEval;
	protected int mult = 4;
	protected int timeInFirst = 0;
	...
	public Bot(World in, String name, Color cellColor) {
		...
		toEval = new CircularZone(new Rectangle(this.getControlling().get(0).getCenterX() - this.getControlling().get(0).getRadius() * mult,
				this.getControlling().get(0).getCenterY() - this.getControlling().get(0).getRadius() * mult, 
				this.getControlling().get(0).getRadius() * 2 * mult, this.getControlling().get(0).getRadius() * 2 * mult), 10);
	...
	public void step() {
		...
		if(isDead()) {
			...
			try {
				...
				controller.setError((in.getLeaderboard().get(in.getLeaderboard().size() - 1).getTotalMass() - this.getTotalMass())/(in.getLeaderboard().get(in.getLeaderboard().size() - 1).getTotalMass()) * (1 - (double)steps/maxAlive));
	...
	public void resetController() {
		...
		this.controller = new MultilayerPerceptron(vect);
	}
